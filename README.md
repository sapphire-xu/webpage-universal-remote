...
Edge / Chrome（Chromium）浏览器扩展：在网页上提供可拖拽的浮层遥控器，用于音视频快进快退、播放暂停，以及上一页 / 下一页。当前版本 **1.4.0**。本扩展按 Manifest V3 编写，在 **Google Chrome 与 Microsoft Edge 上同等支持**（未上架商店，均用「加载未打包的扩展」安装）。
...
- 操作成功但点错了：提示里选「尝试其他可能按键」，或「改为指定某按键」点一次正确按钮并记住  
- 找不到按钮时，提示里还可选「强制执行（有风险）」  
- 「高级」里可调倍速，以及按设定间隔自动点下一页（不会走强制执行）  
